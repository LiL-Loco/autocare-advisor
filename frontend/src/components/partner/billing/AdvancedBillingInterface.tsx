...
const AdvancedBillingInterface: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'plans' | 'usage' | 'billing' | 'payment'>('overview');
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between">
        ...
        {currentSubscription && (
          <div className="flex items-center space-x-3 mt-4 lg:mt-0">
            <span className={`inline-flex px-3 py-1 rounded-full text-sm font-medium border ${getStatusColor(currentSubscription.status)}`}>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div>
                <h3 className="text-xl font-medium text-gray-900">{currentSubscription.plan.name} Plan</h3>
                ...
                <div className="space-y-2">
                  ...
                  {currentSubscription.plan.features.length > 6 && (
                    <p className="text-sm text-gray-500 mt-2">
                      +{currentSubscription.plan.features.length - 6} more features
                    </p>
                  )}
                </div>
              </div>
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              <button
                onClick={() => handleUpgrade(subscriptionPlans.find(p => p.tier === 'enterprise')!)}
          ...
          {usageMetrics && (
            <div className="bg-white rounded-lg border border-gray-200 p-6">
              <h2 className="text-lg font-semibold text-gray-900 mb-6">Usage Overview - {usageMetrics.period}</h2>
              
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {Object.entries(usageMetrics).filter(([key]) => key !== 'period').map(([key, usage]) => (
                  <div key={key} className="text-center">
                    <h3 className="text-sm font-medium text-gray-700 capitalize mb-2">{key}</h3>
                    <div className="relative w-24 h-24 mx-auto mb-3">
                      <svg className="w-24 h-24 transform -rotate-90" viewBox="0 0 100 100">
                        ...
                        <circle
                          ...
                          strokeWidth="8"
                          fill="none"
                          strokeDasharray={`${usage.percentage * 2.51}, 251.2`}
                          className={`${
                            usage.percentage > 80 ? 'text-red-500' :
                            usage.percentage > 60 ? 'text-yellow-500' :
                            'text-blue-500'
                          }`}
                        />
                      </svg>
                      <div className="absolute inset-0 flex items-center justify-center">
                        <span className="text-lg font-semibold">{Math.round(usage.percentage)}%</span>
                      </div>
                    </div>
                    <p className="text-sm text-gray-600">
                      {usage.used.toLocaleString()} / {
                        usage.limit === 'unlimited' ? '∞' : 
                        typeof usage.limit === 'number' ? usage.limit.toLocaleString() : usage.limit
                      }
                ...
                >
                  ...
                  <div className="mt-8 space-y-3 text-sm text-gray-600">
                    <div className="flex justify-between">
                      <span>Products:</span>
                      <span className="font-medium">
                        {plan.limits.products === 'unlimited' ? 'Unlimited' : plan.limits.products.toLocaleString()}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span>API Calls:</span>
                      <span className="font-medium">
                        {plan.limits.apiCalls === 'unlimited' ? 'Unlimited' : `${(plan.limits.apiCalls as number).toLocaleString()}/month`}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span>Storage:</span>
                  ...
                  <div className="mt-8">
                    {isCurrentPlan ? (
                      ...
                      >
                        {currentSubscription && plan.price[billingCycle] > currentSubscription.plan.price[billingCycle]
      ...
      {activeTab === 'usage' && usageMetrics && (
        <div className="space-y-6">
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            <h2 className="text-lg font-semibold text-gray-900 mb-6">Usage Details - {usageMetrics.period}</h2>
            
            <div className="space-y-8">
              {Object.entries(usageMetrics).filter(([key]) => key !== 'period').map(([key, usage]) => (
                <div key={key} className="border-b border-gray-100 last:border-b-0 pb-6 last:pb-0">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-md font-medium text-gray-900 capitalize">{key}</h3>
                    <span className={`px-2 py-1 text-sm font-medium rounded-full ${
                      usage.percentage > 90 ? 'bg-red-100 text-red-800' :
                      usage.percentage > 75 ? 'bg-yellow-100 text-yellow-800' :
                      'bg-green-100 text-green-800'
                    }`}>
                      {usage.percentage > 90 ? 'Critical' : usage.percentage > 75 ? 'Warning' : 'Good'}
                    </span>
                  </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      ...
                      <span>
                        Limit: {usage.limit === 'unlimited' ? 'Unlimited' : 
                               typeof usage.limit === 'number' ? usage.limit.toLocaleString() : usage.limit}
                        {key === 'storage' && usage.limit !== 'unlimited' && ` ${usage.unit}`}
                      </span>
                    </div>
                  ...
                  {usage.percentage > 80 && usage.limit !== 'unlimited' && (
                    <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                      <div className="flex items-start">
                        <svg className="w-5 h-5 text-yellow-400 mr-2 mt-0.5" viewBox="0 0 20 20" fill="currentColor">
          ...
          <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-bold text-gray-900">
                {currentSubscription && selectedPlan.price[billingCycle] > currentSubscription.plan.price[billingCycle]
                  ? 'Upgrade Plan'
                  : 'Change Plan'
                }
              </h3>
            ...
            <div className="space-y-4">
                ...
                {currentSubscription && (
                  <div className="mt-3 text-sm">
                    <p className="text-gray-600">
                      Current: {formatCurrency(currentSubscription.plan.price[billingCycle])}/{billingCycle === 'monthly' ? 'month' : 'year'}
                    </p>
                    <p className="font-medium">
                      {selectedPlan.price[billingCycle] > currentSubscription.plan.price[billingCycle] ? 'Additional' : 'Savings'}: {' '}
                      {formatCurrency(Math.abs(selectedPlan.price[billingCycle] - currentSubscription.plan.price[billingCycle]))}
                    </p>
                  </div>
                )}
              </div>
              ...
              <p className="text-sm text-gray-600">
                Your plan will be {currentSubscription && selectedPlan.price[billingCycle] > currentSubscription.plan.price[billingCycle] ? 'upgraded' : 'changed'} immediately. 
                {currentSubscription && selectedPlan.price[billingCycle] > currentSubscription.plan.price[billingCycle] && 
          ...
          <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            ...
            <div className="space-y-4">
              <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                <div className="flex items-start">
                  <svg className="w-5 h-5 text-red-400 mr-2 mt-0.5" viewBox="0 0 20 20" fill="currentColor">
                    <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
                  </svg>
                  <div>
              ...
              <p className="text-sm text-gray-600">
                Instead of canceling, would you like to downgrade to a lower plan? 
              ...
              <div className="flex flex-col space-y-3">
                <button
                  onClick={() => {
                    const starterPlan = subscriptionPlans.find(p => p.tier === 'starter');
                    if (starterPlan) {
