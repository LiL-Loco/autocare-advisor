...
export function registerTemplateHelpers() {
  ...
  // Format date
  Handlebars.registerHelper('formatDate', function(date: Date, format: string = 'DD.MM.YYYY') {
    if (!date) return '';
  ...
  Handlebars.registerHelper('unsubscribeUrl', function(email: string, type: string = 'all') {
    const baseUrl = process.env.FRONTEND_URL || 'https://autocare-advisor.com';
  ...
  Handlebars.registerHelper('trackClick', function(url: string, messageId: string) {
    const baseUrl = process.env.API_URL || 'https://api.autocare-advisor.com';
...
export class EmailTemplateEngine {
  ...
  renderText(templateContent: string, context: TemplateContext): string {
    try {
      ...
      throw new Error(`Text template rendering failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  renderSubject(subjectTemplate: string, context: TemplateContext): string {
    try {
      ...
      throw new Error(`Subject rendering failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  validateTemplate(templateContent: string): { valid: boolean; errors: string[] } {
    ...
    try {
      ...
      if (unescapedVars) {
        errors.push('Found unescaped variables - ensure they are safe for HTML output');
      }
  ...
  extractVariables(templateContent: string): string[] {
    ...
    if (simpleVars) {
      simpleVars.forEach(match => {
        ...
        if (!variable.includes('(') && !variable.startsWith('this.') && !variable.startsWith('@')) {
    ...
    if (blockVars) {
      blockVars.forEach(match => {
        const parts = match.replace(/\{\{#\w+\s+|\}\}/g, '').trim().split(' ');
