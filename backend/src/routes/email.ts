...
import { createBrevoService, getBrevoService, BrevoWebhookEvent } from '../services/brevoEmailService';
import { EmailTemplate, EmailLog, EmailUnsubscribe } from '../models/Email';
...
router.get('/templates', [
  authenticateToken,
  query('type').optional().isIn(['onboarding', 'marketing', 'transactional', 'nurturing', 'notification']),
  validateRequest
], async (req: Request, res: Response) => {
  try {
...
router.post('/templates', [
  ...
  body('subject').isString().trim().notEmpty(),
  body('htmlContent').isString().notEmpty(),
  body('templateType').isIn(['onboarding', 'marketing', 'transactional', 'nurturing', 'notification']),
  validateRequest
], async (req: Request, res: Response) => {
  try {
    const templateData = {
...
router.get('/analytics', [
  ...
  try {
    ...
    const analytics = {
      ...result,
      deliveryRate: totalSent > 0 ? ((result.delivered / totalSent) * 100).toFixed(2) : 0,
      openRate: totalSent > 0 ? ((result.opened / totalSent) * 100).toFixed(2) : 0,
      clickRate: totalSent > 0 ? ((result.clicked / totalSent) * 100).toFixed(2) : 0
    };
...
router.post('/unsubscribe', [
  ...
  body('type').optional().isIn(['all', 'marketing', 'transactional', 'notifications']),
  validateRequest
], async (req: Request, res: Response) => {
  try {
...
router.get('/health', async (req: Request, res: Response) => {
  try {
    ...
    res.json({
      ...
      message: connectionTest.success ? 'Brevo email service is healthy' : connectionTest.error
    });
