...
router.post('/send-template', [
  ...
  try {
    ...
    const { templateId, recipientEmail, recipientName, variables = {}, options = {} } = req.body;
...
router.post('/send-batch', [
  ...
  body('emails.*.to').isArray().withMessage('Recipients array required for each email'),
  body('emails.*.to.*.email').isEmail().withMessage('Valid email addresses required')
], async (req: Request, res: Response) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
...
router.post('/templates', requireAdmin, [
  ...
  try {
    ...
    if (req.body.syncWithBrevo) {
      try {
        const brevoResult = await brevoService.createBrevoTemplate(template._id.toString());
        
        if (brevoResult.success && brevoResult.brevoTemplateId) {
          // Note: brevoTemplateId would need to be added to EmailTemplate schema
          console.log('Template synced with Brevo, ID:', brevoResult.brevoTemplateId);
        }
      } catch (error) {
        console.warn('[Email API] Failed to sync template with Brevo:', error);
      }
    }
 ...
 */
router.post('/templates/:id/preview', requireAdmin, [
  body('variables').optional().isObject()
], async (req: Request, res: Response) => {
  try {
    const template = await EmailTemplate.findById(req.params.id);
 ...
 */
router.post('/contacts', requireAdmin, [
 ...
 */
router.get('/test-connection', requireAdmin, async (req: Request, res: Response) => {
  try {
...
router.get('/stats', requireAdmin, async (req: Request, res: Response) => {
  try {
    ...
    const stats = await EmailLog.aggregate([
      ...(Object.keys(dateFilter).length > 0 ? [{ $match: { sentAt: dateFilter } }] : []),
      {
        $group: {
          _id: null,
          totalSent: { $sum: 1 },
